 ...
 * @author Colin
 */
public class _23wx_cc extends Novel {
	...
	private Document fetchNovel() throws Exception {
		...
		if (document != null) {
			...
			String href = document.getElementsByClass("bottem2").get(0).getElementsByTag("a").get(3).absUrl("href");
			super.url = href;
			// }
	...
	private Document fetchNovel2Html() throws Exception {
		...
		if (document != null) {
			...
			super.newWriter(super.url2file(super.url));
			{
				...
				for (String line : arr) {
					if (line.length() > 5)
						box.append(String.format("<p class='txt-line'> %s </p>\r\n", line));
				}
